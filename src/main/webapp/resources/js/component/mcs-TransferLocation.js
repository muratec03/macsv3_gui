...
McsTransferLocation.prototype = {
  ...
  */
  showDialog: function() {
    ...
    this.dialog.openCommonPartsDialogButton1NoClose(self.transferLocation, 450, 350, buttonText1, buttonText2, function(buttonNum) {
      if (buttonNum == 0) {
        // OKボタン押下時
  ...
  */
  setLocation: function(filter) {
    ...
    var baseLocationList = [
      [1, language.transferLocationSelect.selects.stocker],
      [2, language.transferLocationSelect.selects.stockerGroup],
      [3, language.transferLocationSelect.selects.ohbGroup],
      [4, language.transferLocationSelect.selects.machine],
//    [5, language.transferLocationSelect.selects.lifter], // MACS4#0016 Del
      [6, language.transferLocationSelect.selects.operatorPort]
    ];

    var baseLabelList = {
      1: [selectController, selectPort],
      2: [language.transferLocationSelect.labels.stockerGroup],
      3: [language.transferLocationSelect.labels.ohbGroup, selectPort],
      4: [language.transferLocationSelect.labels.machine, selectPort],
//    5: [selectController, selectPort], // MACS4#0016 Del
    ...
    this.labelComponent[0].text(language.transferLocationCommon.labels.location);
  ...
  */
  _locationAjax: function(self, innerError, ajaxError) {
    ...
    self.transferLocation = self.selectComponent[0].getValue();

    var data = {
      location: self.transferLocation,
      ioMode: self.ioMode
    };
    self.clearError();
    ...
    callAjax(url, JSON.stringify(data), false, function(retObj) {
      // 成功時
      // 各ラベルをセット
      self._setLabel(self.labelComponent, self.initLabelList, self.transferLocation);
      // ラベルが設定されていない部分を非活性に
      self._setSelectEnable(self.selectComponent, self.initLabelList[self.transferLocation].length);
      // セレクトボックスをセット
  ...
  */
  _aliasAjax: function(self, innerError, ajaxError) {
    ...
    var data = {
      location: self.transferLocation,
      ioMode: self.ioMode,
      id: self.selectComponent[1].getValue()
    };
    self.clearError();
