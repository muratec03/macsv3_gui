...
import net.muratec.mcs.entity.top.ReqGetAlarmListValidateEntity;
import net.muratec.mcs.entity.top.ResGetAlarmListEntity;
...
import net.muratec.mcs.service.top.AlarmService;
 ...
 *              setCsvAlarmList(検索条件の保存を行う)
 *              exeDeleteAlarmList(アラームの削除を行う)
 * @attention
 * @note
 * ----------------------------------------------------------------------------
...
//@formatter:on
@Controller
public class AlarmAjaxController extends BaseAjaxController {

    private static final Logger logger = LoggerFactory.getLogger(AlarmAjaxController.class);
    ...
    /**
     ******************************************************************************
     * @brief     getAlarmList(アラーム情報表示一覧の取得)機能
     ...
     */
    //@formatter:on
    @RequestMapping(value = "/Alarm/GetAlarmList", method = RequestMethod.POST)
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    @OpLog(screenInfo = ComConst.ScreenInfo.INFO_ALARM, logOperationType = ComConst.LogOperationType.GET, number = 3L)
    public ResGetAlarmListEntity getAlarmList(HttpSession session,
            @Valid @RequestBody ReqGetAlarmListValidateEntity reqValidate, Errors errors, Locale locale, Model model)
        ...
        // ------------------------------------
        ReqGetAlarmListEntity reqEntity = ComFunction.ajaxAurgumentCheck(errors, logger, locale, reqValidate,
                ReqGetAlarmListEntity.class);

        // ------------------------------------
        // レスポンスエンティティ生成
        // ------------------------------------
        ResGetAlarmListEntity resEntity = mcsDataTablesService.createResEntity(ResGetAlarmListEntity.class, reqEntity,
        ...
        if (reqEntity.searchDataFlag) {
            ...
            // データ取得、設定
            // ------------------------------------
            resEntity.body = alarmService.getAlarmList(reqEntity);

            // ------------------------------------
            // 全体レコード数取得、設定
            // ------------------------------------
            resEntity.pageInfo.totalRecords = alarmService.getCount(reqEntity);
        }
