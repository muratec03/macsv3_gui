...
import net.muratec.mcs.entity.top.ReqGetPortsListValidateEntity;
import net.muratec.mcs.entity.top.ResGetPortsListEntity;
...
//@formatter:on
@Controller
public class PortsAjaxController extends BaseAjaxController {

    private static final Logger logger = LoggerFactory.getLogger(PortsAjaxController.class);

    // ------------------------------------
    // アラーム情報画面用サービス
    // ------------------------------------
    @Autowired private PortsService portsService;
    ...
    /**
     ******************************************************************************
     * @brief     getPortsList(アラーム情報表示一覧の取得)機能
     ...
     */
    //@formatter:on
    @RequestMapping(value = "/Ports/GetPortsList", method = RequestMethod.POST)
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    //@OpLog(screenInfo = ComConst.ScreenInfo.INFO_ALARM, logOperationType = ComConst.LogOperationType.GET, number = 3L)
    public ResGetPortsListEntity getPortsList(HttpSession session,
            @Valid @RequestBody ReqGetPortsListValidateEntity reqValidate, Errors errors, Locale locale, Model model)
        ...
        // ------------------------------------
        ReqGetPortsListEntity reqEntity = ComFunction.ajaxAurgumentCheck(errors, logger, locale, reqValidate,
                ReqGetPortsListEntity.class);

        // ------------------------------------
        // レスポンスエンティティ生成
        // ------------------------------------
        ResGetPortsListEntity resEntity = mcsDataTablesService.createResEntity(ResGetPortsListEntity.class, reqEntity,
        ...
        if (reqEntity.searchDataFlag) {
            ...
            // データ取得、設定
            // ------------------------------------
            resEntity.body = portsService.getPortsList(reqEntity);

            // ------------------------------------
            // 全体レコード数取得、設定
            // ------------------------------------
            resEntity.pageInfo.totalRecords = portsService.getCount(reqEntity);
        }
