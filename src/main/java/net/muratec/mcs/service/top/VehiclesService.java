...
import net.muratec.mcs.entity.top.VehiclesListEntity;
...
//  @Autowired private AmhsMapper amhsMapper;
    ...
    //@formatter:on
    @Transactional
    public int getCount(ReqGetVehiclesListEntity reqEntity) {

        return vehicleMapper.getCount(reqEntity);
    }
    ...
    //@formatter:on
    @Transactional
    public List<VehiclesListEntity> getPortsList(ReqGetVehiclesListEntity reqEntity) {

        List<VehiclesModel> recList = null;

        recList = vehicleMapper.selectList(reqEntity);

        if (recList == null) {
            return null;
        }

        List<VehiclesListEntity> retRecList = new ArrayList<VehiclesListEntity>();
        for (VehiclesModel vehicle : recList) {
        	
        	VehiclesListEntity entity = new VehiclesListEntity();
            
            entity.vehicleId = vehicle.getVehicleId();
            entity.llcName = vehicle.getLlcName();
            entity.alarmText = vehicle.getAlarmText();
            entity.vehicleKind = vehicle.getVehicleKind();
            entity.vehicleState = vehicle.getVehicleState();
