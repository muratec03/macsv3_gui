...
import net.muratec.mcs.mapper.CarrierShapeMapper;
import net.muratec.mcs.mapper.CarrierTypeMapper;
import net.muratec.mcs.mapper.DefaultProcessMapper;  // プロセスデフォルト対応 - 制御側未対応のため、未使用
import net.muratec.mcs.mapper.EmptyCarrierContamiMapper;
import net.muratec.mcs.mapper.EmptyCarrierControllerMapper;
import net.muratec.mcs.mapper.EqpTypeMapper;
import net.muratec.mcs.mapper.GrpMapper;
//import net.muratec.mcs.mapper.HostGrpMapper;  // Hostポートグループ対応 - 制御側未対応のため、未使用
...
import net.muratec.mcs.mapper.ProcessMapper;
//import net.muratec.mcs.mapper.PurposeTypeMapper;      // MACS4#0099 Del
import net.muratec.mcs.mapper.RouteMapper;
import net.muratec.mcs.mapper.ScreenMonitorMemberMapper;
import net.muratec.mcs.mapper.StringMasterMapper;
import net.muratec.mcs.mapper.TakeoverRuleRltMapper;
import net.muratec.mcs.mapper.UnitMapper;
import net.muratec.mcs.mapper.VipMapper;
import net.muratec.mcs.mapper.ZoneMapper;
import net.muratec.mcs.model.Amhs;
import net.muratec.mcs.model.AmhsExample;
import net.muratec.mcs.model.AmhsModel;
import net.muratec.mcs.model.DefaultProcess;         // プロセスデフォルト対応 - 制御側未対応のため、未使用
import net.muratec.mcs.model.DefaultProcessExample;  // プロセスデフォルト対応 - 制御側未対応のため、未使用
...
//import net.muratec.mcs.model.HostGrp;         // Hostポートグループ対応 - 制御側未対応のため、未使用
//import net.muratec.mcs.model.HostGrpExample;  // Hostポートグループ対応 - 制御側未対応のため、未使用
...
import net.muratec.mcs.model.OhbMCS;
...
//    @Autowired private ScreenMonitorMemberMapper screenMonitorMemberMapper;// MACSV2→MACSV4対応                         天津村研　董
    ...
    /**
     ******************************************************************************
     * @brief     CARRIERテーブルを全件取得し、selectBox用配列で返す
     * @param
     * @return    [[ACTION_FLAG][ACTION_FLAG]]
     * @retval
     * @attention
     * @note    ACTION_FLAGのみを取得
     * ----------------------------------------------------------------------------
     * VER.        DESCRIPTION                                               AUTHOR
     * ----------------------------------------------------------------------------
     ******************************************************************************
    ...
    //@formatter:on
    @Transactional(readOnly = true)
    public List<Short[]> getcarrierActionBox() throws McsException {

        List<Short> actionFlagList = carrierMapper.getActionFlag();

        List<Short[]> selBoxList = new ArrayList<Short[]>();

        for (Short actionFlag : actionFlagList) {
...
//        ohbExample.createCriteria().andOhbTypeNotEqualTo(ComConst.OhbType.CODE_IFOHB);//20191219 dqy del
    ...
    /**
     ******************************************************************************
     * @brief     EQP_TYPEテーブルを全件取得しSelectBox用配列で返す
     * @param
     * @return    [[EQP_TYPE][EQP_TYPE]]
     * @retval
     * @attention
     * @note
     * ----------------------------------------------------------------------------
    ...
    //@formatter:on
    @Transactional(readOnly = true)
    public List<String[]> getEqpTypeBox() {

        EqpTypeExample amhsExample = new EqpTypeExample();
        amhsExample.createCriteria();
        amhsExample.setOrderByClause("EQP_TYPE ASC");

        List<EqpType> eqpTypeList = eqpTypeMapper.selectByExample(amhsExample);

        List<String[]> selBoxList = new ArrayList<String[]>();

        for (EqpType eqpType : eqpTypeList) {
            String[] data = new String[2];
            data[0] = eqpType.getEqpType();
            data[1] = eqpType.getEqpType();
            selBoxList.add(data);
        }
